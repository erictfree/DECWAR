...
import { romulanText as T,constants as K } from '../../src/runtime/variant-values.ts';
...
export function bindRomulanSpeechRuntime(f:Host){
  const broadcastOnly=currentVariant().definition.id==='austin';
  ...
  function* call(entry:RomulanSpeechEntry):Generator<string,void,void>{yield*f.cpu.pushP(1000n);yield*romulanSpeechRuntime(entry,f.m,f.r,f.rt.args,s,io,broadcastOnly);assert.equal(yield*f.cpu.popP(),1000n);}
