...
export type PregameStatementMessage='strtup'|'pgame1'|'honorInstruction'|'documentInstruction'|'documentInstructionEnd'|'documentMessage';
...
export function* pregameStatements<W>(low:CommonBlock,l:PregameStatementLocals,io:PregameStatementServices<W>):Generator<W,void,void>{
  ...
  for(;;){
    ...
    if(io.logical(yield*io.equal(low.address('tknlst',1),'HONORROLL'))){yield*io.invoke({routine:'shosta',argument:true});continue;}
  ...
  yield*io.out('pgame1',1);yield*io.out('honorInstruction',1);yield*io.out('documentInstruction',1);yield*io.out('documentInstructionEnd',1);
  for(;;){
    ...
    if(n===2n){yield*io.out('documentMessage',1);continue;}
