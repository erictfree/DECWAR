...
export function duelRoute(map: ObservedMap, scan: Scan, p: Position, goal: Position, now: number, team: 'FEDERATION' | 'EMPIRE' = 'EMPIRE'): Position | undefined {
  const enemyPlanet = team === 'FEDERATION' ? '@E' : '@F';
  const enemyBase = team === 'FEDERATION' ? ')(' : '<>';
  const safe = (q: Position) => scan.cells.some(c => distance(c, q) === 0 && c.symbol === ' .') && !map.danger(q, team, now) && !scan.cells.some(c => [' @', enemyPlanet].includes(c.symbol) && distance(c, q) <= 2);
  ...
  for (const cell of map.cells.values()) {
    const radius = cell.symbol === enemyBase ? 4 : [' @', enemyPlanet].includes(cell.symbol) ? 2 : 0;
  ...
  let next = route(map, p, goal, team, now);
  ...
  if (next && safe(next)) {
    ...
    for (let n = 2; n <= 4 && distance(next, goal); n++) {
      const following = route(map, next, goal, team, now);
