...
for (const team of ['EMPIRE', 'FEDERATION'] as const) for (const start of [{ v: 30, h: 10 }, { v: 22, h: 15 }]) {
  test(`duel setup escapes defended terrain for ${team} from ${start.v},${start.h}`, () => {
    const objects = [{ v: 25, h: 10, symbol: team === 'EMPIRE' ? '<>' : ')(', radius: 4 }, { v: 19, h: 14, symbol: ' @', radius: 2 }, { v: 19, h: 17, symbol: ' @', radius: 2 }, { v: 15, h: 10, symbol: team === 'EMPIRE' ? ')(' : '<>', radius: 0 }, { v: 21, h: 11, symbol: ' *', radius: 0 }];
    ...
    for (let step = 0; step < 70 && distance(p, goal); step++) {
      ...
      const next = duelRoute(map, scan, p, goal, now, team); assert.ok(next, 'route must progress');
