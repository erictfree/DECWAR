# Implementation progress through September 5, 2026

Historical record copied from the former status document. Its checkpoint counts,
open dependencies and operational ports describe different moments in development;
they are not the current release specification. See [current status](../status.md).
The record below is preserved with its original statements; only the relative
implementation-ledger link has been adjusted for this location.
