...
| EX-MODEL-15 | Undocked ship with 1 torpedo, 1000 energy units, shields down at 20%, hull damage 200 units; exactly one friendly planet adjacent, positive faction captured-planet count, and no adjacent friendly base; DOCK | Before turn accounting: 6 torpedoes, 1500 energy units, shields still down at 30%, hull damage 150 units; docked, green, life reserve 5. A turn with automatic repair follows. |
...
| EX-COMP-39 | PAYING admission; shared game number 0; stored PAYING game number 12; selected ship mission count 4; ordinary access | Assign shared and selected stored game number 13; increment the ship's mission count to 5. No Honor Roll entry is created and reservation follows the numbering operation. |
| EX-COMP-40 | NON_PAYING admission; shared game number 7; PAYING stored game number 12; NON_PAYING stored game number 8 and mission count 4; all accesses succeed | Shared game number becomes 12. Write PAYING statistics without a mission increment, then write NON_PAYING game number 12 and mission count 5. |
| EX-COMP-41 | PAYING numbering reaches final destination opening; that opening fails and subsequent reporting returns normally | Release statistics access, report and return the computed numbers, and let the admission caller continue. The announcement does not certify a saved mission increment. |
| EX-COMP-42 | Explicit HONORROLL; nonempty group; declared report width 79 columns; no interrupt | Emit the column heading including Ship, Runtm and Date, but omit those fields from rows. At width 80 include them in rows. |
| EX-COMP-43 | Both factions have primary records; interrupt becomes pending during the first faction's primary rows and remains pending | No per-row check is added. Omit the first faction's memorial group; the other primary introduction can still appear, but its heading and rows are omitted. Consume the interrupt at source completion. |
| EX-COMP-44 | NON_PAYING source has four empty groups; no pending interrupt; explicit HONORROLL | Emit no overall or non-paying-source heading for that source, then attempt PAYING. Empty-source omission does not cancel the second-source rule. |
| EX-COMP-45 | Honor Roll score 150 points; elapsed 90 seconds; width 80 | Credits displays 2 and Runtm displays 2 in their fixed fields. Ranking still compares the unrounded score and elapsed duration. |
| EX-COMP-46 | Honor Roll score -160 points | Credits displays -2 under generalized signed rounding; do not alter the recorded score or ranking. |
| EX-COMP-47 | Wide Honor Roll row; ship-name prefixes ABC and ENTERPRISE | ABC occupies a nine-character padded field; ENTERPRISE occupies ten. The elapsed field follows immediately after that field. |
| EX-COMP-48 | Honor Roll account member labels 7 and 123456 | Append four spaces after 7 and one space after 123456 before the Credits field. Account identity is unchanged. |
| EX-COMP-49 | CompuServe actor at (20,20), energy 1000 units, shields down, docked, no tractor beam, undamaged devices; MOVE ABSOLUTE 22 20 has a clear path, then destination coordination entry fails | RelocationRefused before subsequent command acquisition: position remains (20,20), energy is 984 units, condition GREEN and undocked. Stardate and devices unchanged; no movement turn or automatic repair. Move adds no response after the refusal; coordination output is governed separately. |
