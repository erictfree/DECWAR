...
export function lockLayout(){
  const lines=sourceFile('WARMAC.MAC').split('\n'),input=inputRuntimeLayout(),file=fileLayout();
  const count=sourceFile('WARMAC.MAC').match(/knloks==\^d(\d+)/i);if(!count)throw new Error('Missing KNLOKS');
  const maximum=Number(count[1]),start=lines.findIndex(l=>/^tobcb:/.test(l));
  const expected=[['tobcb','3',3],['tobuf','3',3],['ttyBuffer','50',40],['frebie','1',1],['ftlerr','1',1],['locked','1',1],['svlock','1',1],['whohas','3',3],['loktab','knloks',maximum],['jsqwho','1',1],['timsta','^d50',50],['timlcn','^d50',50]] as const;
