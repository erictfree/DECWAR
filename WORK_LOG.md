...
## 2026-09-08 — movement and docking parity suite

Added five public-observation cases under experimental/parity: invalid coordinates, excessive impulse range, one-sector warp, one-sector impulse and docking after a bounded scan-based approach. Separate state-contract and output results retain before/after STATUS, scan observations, approach steps and raw wire evidence. Missing setup is incomplete, never a pass; matching failures remain failures. Source: Austin DECWAR.FOR MOVE/IMPULS2141–2254 and DOCK893–938; MSG.MAC erloc8:83 and move1B:132. No runtime/legacy edits.

Eight harness tests, scoped TypeScript checking and source audit passed (logs/parity-behavior/tests-final.txt, typecheck-final.txt, audit.txt). Live paired run is recorded under logs/parity-behavior/live/. Fresh healthy ships mean damaged-device repair and ammunition refill remain unexercised, even if their capped postconditions hold. Native clocks and random draws are not aligned.

First behavior run preserved as incomplete: native docking approach stopped on an enemy base at range10 despite a healthy ship. Refined setup to use source base radius4 and retained planet radius2, with mobile/unknown targets still excluded. Retry revealed an incorrect harness undocking assertion: MOVE invalid coordinates returns before label700 and preserves a prior docked flag; IMPULS range rejection clears it after label700. Corrected evaluator to recompute all checks from command/before/after evidence, retaining old recorded booleans unchanged (source-contracts-v2). Added boundary and rejection-order tests; ten tests pass. Final reports must be produced by review-behavior against the retained captures rather than trusting earlier capture-time booleans.

Final reanalysis: all five source-relative contracts and action outputs match after explicit echo removal, with no capture/cleanup errors. Native approach completed20 moves; TypeScript26. Both measured engine steps cost8 displayed energy units. DOCK restored TS4776→5000 and native4824→5000. Authoritative artifacts: logs/parity-behavior/retry/reviewed.json and reviewed.md; initial report retained and superseded by source-contracts-v2. Ten tests, scoped typecheck, source audit and whitespace checks passed. Native reference remains on2030; dedicated TS2424 test server stopped after capture. No main game behavior changed.

## 2026-09-08 — capture and construction parity suite

Extended the runnable paired Telnet harness with objectives: BUILD before ownership, neutral CAPTURE, builds 1–4, fifth-build conversion or capacity refusal, and conditional docking at the new base. Setup uses public LIST/SCAN/STATUS with a bounded 60-step approach and fresh adjacent target checks. Source: Austin DECWAR.FOR BUILD523–591 and CAPTUR600–684; neutral capture invokes defensive PHADAM, while BUILD has no direct energy debit. No runtime or preserved-source changes. Recomputed state checks and raw output comparison remain separate; skips cannot imply successful parity.

Live TypeScript playable versus native SIMH f78f2ec/FORTRAN6 finished with seven matching state contracts and no capture/cleanup errors. Six responses match after explicit command-echo removal. Capture coordinates and random defensive hits differ (TS18.7/native9.3), retained in evidence. Both worlds had ten friendly bases: fifth BUILD correctly refused, so conversion and new-base docking remain unverified and overall outcome is incomplete (exit2). Both planets remain captured with four builds in these test worlds. Docker daemon was unavailable; this verifies native SIMH only.

Evidence: logs/parity-objectives/live/{typescript,pdp10}.jsonl, report.json, report.md and manifest.json; server logs/data remain under logs/parity-objectives/. Checks: node --test experimental/parity/test/*.test.ts (15 passed, tests-final.txt); scoped npx tsc --noEmit --ignoreConfig --target esnext --module nodenext --allowImportingTsExtensions --strict --types node on run.ts, capture-objectives.ts and objectives.test.ts (typecheck-final.txt); npm run audit:check (audit-final.txt); git diff --check (diff-check.txt). Updated current guide/status and runnable command example. Next fixture needs a free friendly base slot to test successful conversion and docking; broader combat, capture of developed/enemy planets, resource replenishment and Docker deployment remain separate coverage.

## 2026-09-08 — tournament seed repeatability observations

Tested whether the same tournament ID yields matching initial public state. Added capture-seed.ts (fresh-world seed selection required, no movement/weapons) and review-seed.ts (reparse raw STATUS/BASES/SCAN, validate four complete seeded captures, compare repeated and cross-engine starts). Source: Austin SETUP.FOR169–193 selects SETRAN for new worlds; WARMAC.MAC2285–2324 implements the game generator and clock fallback for zero. Used nonzero seed1729, Romulans off, black holes off, Federation Yorktown. No runtime/source modifications.

Booted two isolated copies of the existing native reference disks on2031/2032, leaving the existing2030 game alone. Created two fresh TypeScript playable hosts on2424/2425. All four observed starts matched exactly: position7–2, ten friendly-base coordinates,204 initial scan cells. All selected TOURNAMENT1729 and completed logout without errors. This is matching initial public observations, not a full galaxy dump, other-seed verification or later combat/random-draw alignment. Native SIMH used; Docker remains untested. All four temporary hosts stopped afterward.

Evidence: logs/parity-seed/report.json, {ts-a,ts-b,pdp-a,pdp-b}.jsonl, environment.json, copied boot logs/configuration and capture/server logs. Scoped TypeScript check passed (typecheck.txt); npm run audit:check passed (audit.txt); reviewer rejected missing seed selection and detected altered raw scan despite unchanged derived fields (reviewer-checks.txt; synthetic fixture files clearly named). git diff --check passed (diff-check.txt). Current guide/status updated. Next: full static-map comparison if stronger galaxy equality is needed; initial matching observations justify further seeded parity work but do not prove identical battles.

## 2026-09-08 — complete seeded board comparison

Advanced the seeded-start milestone from 204 public scan cells to the entire packed BOARD. All 1,875 words / 5,625 cells match between TypeScript playable and native SIMH for seeds1729,42,8675309, with Romulans and black holes off. Includes object indices and initial Yorktown placement. Each board independently reproduces its Telnet scan. This proves those initial boards, not every game-state field, other seeds/options or later random/combat alignment.

Added a temporary in-process TypeScript host/capture and a native debugger-log reviewer. Source addresses: Austin HISEG.FOR21, pinned DECWAR.MAP47 (HISEG400010), generated BOARD offset425/length1875; WARMAC.MAC813–815 and4408–4422 pack three12-bit cells per word. Optional capture-seed gate holds the native session for up to five minutes. On isolated SIMH copies, break at GTKN460405 before departure, read UB and its direct section/page mappings, then examine the physical words for virtual400661–404403. Reviewer derives physical pages solely from native pointers (bundled kx10_cpu.c2419–2542), rejects unsupported/missing mappings, reparses public scans, and compares all cells. No deposits or game-source/runtime edits. Native mapping differed for the third seed; no physical address reuse assumption.

Tracking retained diagnostic attempts: cached virtual examine while idle returned no usable mapping; resolved through the stopped job's page table. Unsupported examine redirection attempts are in the debugger log; console-log rotation flushes the successful physical dump. The first TypeScript snapshot omitted BOARD's required subscript; corrected to address(board,1), failed output retained in ts-1729-console.txt. An initial esnext typecheck exposed an existing Generator iterator-helper mismatch in variant-execution.ts; the check passes with the repository's ES2023 target (typecheck-final.txt). No changes made to that runtime file. One console EXAMINE arrived before the breakpoint and was rejected by the TOPS-10 monitor; subsequent extraction waits for the confirmed breakpoint. All three native sessions completed cleanup; isolated emulators and temporary TypeScript hosts stopped. Existing reference2030 was untouched.

Evidence: logs/parity-full-map/report-{1729,42,8675309}.json; native-*-debugger.txt, native-*.jsonl, ts-*.json and command/error logs. Checks:15 harness tests pass (tests.txt); full scoped TypeScript check passes with targetES2023; audit:check passes (audit.txt). Reviewer diagnostics detect an altered75,75 cell outside the public scan and reject a missing/invalid native mapping (reviewer-checks.txt, explicitly synthetic fixture files). Procedure documented in experimental/parity/BOARD-SNAPSHOTS.md; guide/status updated. Next useful parity step is controlled action sequences from these matching seeded initial boards, keeping subsequent action/timing and random-stream divergence visible. Native debugger collection remains manual; Docker and automated full-board extraction are not claimed.

## 2026-09-08 — seeded action sequence and nova parity

Added capture-sequence.ts, sequence.ts and review-sequence.ts to compare identical fixed commands from fresh seed1729 starts (Austin playable vs isolated native SIMH; Romulans/black holes off). Default sequence: MOVE7,3; DOCK; SHIELDS DOWN; MOVE8,3; SHIELDS UP; TRANSFER-100; TRANSFER100; IMPULSE7,3; DOCK; invalid MOVE0,0. Source contracts: MOVE/IMPULS2141–2254, DOCK893–938, SHIELD3739–3806. All ten state contracts and forty action/STATUS/DAMAGES/SCAN responses match after only explicit command-echo removal; all initial reports also match. Stardates are retained, not normalized. Both sessions logged out cleanly. This advances matching initial boards into verified selected connected actions.

Continued with a fresh star-shot mode: TORPEDOES ABSOLUTE1 11 1, source TORPED4301–4325 and SNOVA3806 onward. Both engines reported Star@11-1 novas, consumed one torpedo (10→9), changed that scan cell to empty, and returned identical STATUS/DAMAGES/SCAN. The star outcome is compared, not hardcoded as a required nova. This is one observed random-dependent match; it does not prove all random draws or ship-versus-ship combat. No runtime or immutable-source changes. Isolated native2031 and TS2424 test hosts stopped after clean logout; existing native2030 left alone.

Evidence: logs/parity-sequence/{ts,native,shot-ts,shot-native}.jsonl; report.json; shot-report.json; nova-check.txt; native boot logs, TS host logs and stdout captures. The in-progress-report.json is explicitly incomplete and retained; final report.json supersedes it. Checks:15 harness tests pass (tests-final.txt); scoped ES2023 TypeScript check passes (typecheck-final.txt); audit:check passes (audit-final.txt). Synthetic reviewer fixtures verify that stardate differences remain visible and a missing action yields incomplete (reviewer-checks.txt). Updated current guide/status; all substantive results and limitations recorded. Next: controlled ship combat with paired attackers/targets, then capture/build under aligned seeded setups; full conversion still needs a free base slot. Docker remains unverified.

## 2026-09-08 — controlled seeded ship encounter

Started paired attacker/target testing from seed1729. Later arrivals use clock-seeded spawn positions (SETUP.FOR169–193), so the target travels through public scanned space to the common Empire base15,10, docks at14,10 to restore supplies, then takes four fixed moves to10,10. Yorktown takes ten fixed moves from7,2 to10,9. Planned pre-fire energy: attacker4920, target4968, intact target shields/devices. No memory deposits or target strategies; all actions scripted through Telnet. Source weapon commands follow PHASER/TORPED in Austin DECWAR.FOR; comparison keeps raw reports and distinguishes state from setup-dependent clocks/scores.

Initial setup failure retained in logs/parity-duel/: warp6 optimization overheated the engines and kept retrying an unavailable speed. Source MOVE2179–2216 allows overheating above4. TypeScript exhausted its approach budget; the stalled native capture was terminated and its isolated emulator stopped, discarding those test jobs. Corrected setup to maximumwarp4 and fail on no movement progress; restarted fresh isolated hosts for retry under logs/parity-duel/retry/. No game-runtime repair made. Existing reference2030 remains separate. Retry and final combat evaluation are in progress.

Setup follow-up: the first warp4 TypeScript route correctly refused entry near a neutral planet, exposing mismatched planner/execution exclusions; the older native planner also circled a defended base. Replaced greedy movement with compressed straight runs of the existing A* route, blocked the same known defense radii during planning, and kept speed<=4. Added two route regressions for the observed base/planet traps; both reach the staging base without crossing defended cells. TypeScript then reached the base healthy but needed a second DOCK: DOCK restores at most1000 energy per call. Added bounded repeated docking until5000 and complete state verification. Further captures under supplied/ use that corrected setup. Earlier attempts remain as incomplete evidence; isolated native retries were explicitly terminated/rebooted. One retry boot-log copy used the wrong relative working directory and failed; the Telnet captures remain intact.

TypeScript supplied encounter completed: phasers reduced target shields100→80.1% with0 hull damage; attacker energy4920→4540 (180 shot plus200 high-speed shield-control cost, PHASER2685–2687,2750). Torpedo then hit for103.8, displaced Wolf to9,11, reduced shields to67.5%, and ammunition10→9. Native supplied encounter still travelling to the base. No cross-engine combat conclusion yet.

Completed supplied pair: canonical initial/post-phaser/post-torpedo state, device damage, total score and hit output match. Raw target STATUS differs in setup stardate, explicitly excluded from canonical comparison and retained in raw comparisons. Evidence: logs/parity-duel/supplied/report.json. A repeat extended-torpedo run first diverged on the first torpedo: native94.9 versus TypeScript103.8 despite matching visible pre-fire state and phaser outcome. Cause is unresolved; no general random-stream or combat equivalence claim. Both extended targets survived nine additional shots (displacement and lost torpedoes limited damage), so destruction/report.json remains incomplete. Both roles logged out; isolated2032 and its TS2425 host stopped. Original2030 was not touched.

Added optional stationary-unshielded-phasers-v1 destruction policy: after the same two-shot prefix, lower target shields and fire bounded180-unit phaser shots at its freshly observed position. Require native/TS reentry prompt, absence from USERS and target cleanup before declaring lifecycle coverage. Source: PARAM.FOR KENDAM25000 internal, PHASER/PHADAM and SETUP.FOR76–113 PREGAME/QUIT. First TypeScript stationary capture stalled after DOCKED without a command prompt; capture/cleanup timeout retained under stationary/, host stopped without a runtime change. Fresh isolated TypeScript retry under stationary-retry/ proceeds alongside the native stationary run. This unexplained stall remains diagnostic evidence.

Reviewer now reports first observed canonical-state/hit divergence even when a later stage makes the overall run incomplete. Raw-state mutation and missing-cleanup negative checks pass (stationary-retry/reviewer-checks.txt, synthetic fixtures explicitly named). Seventeen harness tests, scoped ES2023 TypeScript checks and audit:check pass (stationary-retry/tests.txt, typecheck-final.txt, audit.txt). No game runtime or preserved archive was modified.

Stationary destruction completed on both engines with no capture/cleanup errors: native required four follow-up phaser shots; TypeScript five. Both recorded the exact reentry prompt, absence of Wolf from USERS and target cleanup (native PREGAME→QUIT→K/F verified). Final attacker energy3020/native versus2640/TS; total score3018.8/native versus3551.9/TS. Report correctly returns differences (exit1), first divergence at prefix TORPEDOES ABSOLUTE1 10 10. Authoritative evidence: logs/parity-duel/stationary-retry/report.json compares stationary-retry/ts.jsonl with stationary/native.jsonl. Earlier stationary/ts.jsonl is failed evidence, not the paired final TS capture. Both isolated hosts stopped; boot log and final listener inventory retained. Documentation now states selected matched combat, repeat divergence and verified destruction lifecycle separately. Next investigation: capture hidden per-job random state immediately around the first differing torpedo before attributing it to RNG ordering, arithmetic or timing. No speculative gameplay repair made.

## 2026-09-08 — first torpedo divergence investigation

Tracing the observed103.8/94.9 torpedo difference. Austin DECWAR.FOR4099–4108 consumes rand,rana and hit RAN values before shield absorption; visible pre-shot state alone cannot establish those inputs. Added isolated experimental diagnostic host wrapping existing random methods to record private seed before/after, argument, result and caller without substituting values. Added optional bounded pre-torpedo gate to capture-duel for native debugger observation. No runtime or archive changes. Evidence accumulating in logs/parity-torpedo-trace/. Initial wrapper typecheck failed on a union return type; corrected with a generic wrapper, final scoped check passes. Native SIMH trace initially examined register bank0, which is not the active user bank; corrected to record FMSEL and FM array entries. Those initial diagnostic attempts are retained, not used as seed evidence.

Reproduced native94.9 with debugger trace and TypeScript103.8 with random wrapper. Both phaser calls start from seeds310313303400 and177421534400 (octal). Native then consumes20 draws at the PLNATK caller427543 before TORP; TypeScript does not. Source Austin main223–239 increments shared HISEG DOTIME and runs defenses each NUMPLY actions. PLNATK2807–2811 consumes neutral-planet IRAN(2) before range checks. Unequal target setup action parity explains the mismatch: supplied pair target stardates24/26 both even, earlier extended pair20/27 opposite, new traced pair32/13 opposite. No timer/RNG arithmetic repair justified.

Added review-random-window.ts: recognizes pinned native user-register bank020 (FM[17] is AC1; FM[30] ARG carries the compiler call-block address), groups by UB, checks consecutive seeds using existing integer RNG, and reconstructs noncritical shielded damage using existing rounded-word operations. Inputs yield1038/949 internal exactly. Evidence: random-window-report.json, native-debugger.txt, native-random.json (intermediate), random.jsonl, ts.jsonl, native.jsonl and paired report.json. Native capture and both logouts completed; traced emulator stopped. Source numerical policy and archives unchanged.

Corrected capture setup with even-target-turns-v1: count travel and docking actions, add a full-supply DOCK if odd, then verify restored state before fixed approaches. This aligns shared activity phase via public commands, without memory writes. Reviewer rejects differing policies and recomputes setup action counts. Diagnostic host also records DOTIME/NUMPLY. Full aligned destruction pair running under logs/parity-torpedo-trace/aligned/. Native retained original2030 separately. Seventeen harness tests and source audit passed before the phase correction; updated scoped typechecks pass; final checks follow the live rerun.

Added RANDOM-TRACE.md with the pinned breakpoint/register procedure and limitations. Scoped typechecks,17 harness tests and audit:check pass under aligned/{typecheck-final,tests-final,audit-final}.txt. Reviewer accepts a synthetic valid phase and rejects changed phase count or a missing docking record; random-window reviewer rejects an altered native seed. Synthetic artifacts are explicitly named and are not reference evidence. One inline negative-check command had an extra closing brace; retained as aligned/phase-reviewer-checks-failed.txt before successful retry. Aligned native setup exercised the added full-supply docking action and recorded24 setup turns; final measured encounter is still running. TypeScript aligned capture completed and its isolated diagnostic host was stopped.

Aligned native and TypeScript destruction run completed: report.json returns matched-combat-state-and-hit-output with zero capture/cleanup errors. All seven weapon responses, canonical ship/device states and total scores match; initial torpedo103.8, then five follow-up phaser shots to death. Both attackers finish energy2640 and points3551.9. Both targets reached reentry, disappeared from USERS and completed cleanup. Native extra phase DOCK was exercised. Source stardates still differ and are retained in raw comparisons; this is selected encounter verification, not universal parity. The earlier incomplete in-progress-report.json is retained and superseded. Temporary native2031 and TS2424 stopped; original2030 remains (aligned/final-listeners.txt), native boot log preserved. Final scoped typecheck and whitespace check pass. Updated public guide/status and diagnostic procedure. Concrete torpedo discrepancy resolved in the harness; no game-runtime or immutable-source changes. Broader autonomous-phase/state alignment and the earlier unrelated docking stall remain separate future work.

Publication status: automatic approval review rejected the combined commit/push twice because GitHub origin ownership/trust was not established to its satisfaction. Verified origin is https://github.com/erictfree/DECWAR.git and prior successful push evidence names the same destination, but the second rejection persisted. Local commit proceeds separately; push requires explicit destination approval. No remote publication from these attempts.

## 2026-09-08 — base replacement parity fixture

Continuing the incomplete conversion milestone. Source Austin TORP4330–4343 prevents friendly ship/base damage, so use the opposing Federation attacker to destroy Empire base15,10 from range6, then move the Empire builder from10,10 to neutral planet19,14. Existing seeded two-player setup and activity-phase alignment are reused. Added an installations mode: bounded180-unit phaser siege with shields lowered, public empty-cell/base-count evidence, neutral CAPTURE, five BUILDs, and DOCK at the replacement base. Source BUILD523–591, CAPTUR600–684, DOCK893–938, PHACON/PHADAM. No game-runtime or preserved-source changes. First TypeScript capture running under logs/parity-installations/; isolated native2031 booting, original2030 preserved. Reviewer reparses raw reports and separates construction/docking contracts from unequal random-dependent output; successful conversion requires base count9→10 and planet-list removal.

TypeScript installation run completed cleanly:10 siege shots destroyed Empire base15,10; Wolf captured19,14, performed five builds, and docked at the new base. All eight source-relative contracts pass when raw reports are reparsed. Reviewer self-comparison is explicitly synthetic, not native evidence. Negative fixtures reject missing conversion, missing cleanup and an altered new-base coordinate despite unchanged derived fields (reviewer-negative-checks.txt). PLANETS needs its own row extraction because the existing default LIST parser requires ship rows; reviewer now parses the owned planet row directly. Seventeen harness tests, scoped typecheck and source audit passed. Native setup remains in progress; no cross-engine result claimed yet.

Native siege matched all ten TypeScript phaser responses, including emergency shield recovery followed by destruction on shot10. Empire BASES now shows nine surviving bases. Native builder proceeding to19,14; construction/docking still pending. Source-relative comparison deliberately retains builder energy/hull/shield/stardate before and after each measured action instead of equating independently seeded builder random streams.

Completed paired installation fixture with zero capture/cleanup errors. All eight source-relative contracts pass on both engines. All ten siege responses, five BUILD responses and DOCK match after command-echo removal (16/17 measured action outputs); CAPTURE retains5.1 versus5.0 defensive-hit output with unaligned builder random streams. Fifth BUILD removes the planet row and replaces the lost base, verifying Empire count10→9→10. New-base docking restores TS4898.8/native4898.9 energy to5000; raw reports also show hull damage cleared and shields restored. The authoritative report is logs/parity-installations/report.json, outcome differences (exit1), not incomplete. Captures/resources retain the unequal damage and stardates. Current guide/status updated. Checks:17 harness tests, scoped final typecheck, source audit, reviewer self/negative checks and final raw-contract check all pass; artifacts are under logs/parity-installations/. Isolated native2031 and TS2424 hosts stopped after clean logout, original2030 untouched; native-boot.txt and final-listeners.txt preserved. This closes the selected successful conversion/new-base docking gap. No game runtime or immutable-source changes. Publication remains pending the previously requested GitHub destination approval; this checkpoint is local only.

## 2026-09-08 — docking stall midnight and heap evidence

Investigated the saved stationary duel failure without changing game code or restarting hosts. `logs/parity-duel/stationary/ts-console.txt` confirms a roughly 4 GB heap exhaustion, rather than merely an unrecognized prompt. The target sent DOCK at 2026-09-08T23:59:59.950Z; timeout followed at 2026-09-09T00:01:59.957Z after DOCKED output.

Austin `legacy/utexas/WARMAC.MAC:3373-3401` PAUSE computes a raw time-of-day deadline and compares subsequent MSTIME directly, unlike ETIM's midnight correction at 3362-3368. `src/compat/wait-runtime.ts` retains this behavior and live clock bindings use UTC milliseconds since midnight. Existing bounded regression `Raw PAUSE retains midnight rewait behavior without ETIM correction` passes; output saved to `logs/parity-stall-midnight/raw-pause-test.txt`. This demonstrates the raw wait cannot reach a deadline beyond 86400000 with wrapped time-of-day readings, but is not yet a full reproduction of this session's failure.

Possible amplification: `src/runtime/session.ts` immediately resolves delay waits after disconnect, while the retained wait fixture appends events and operands at each HIBER. A stuck raw PAUSE could therefore spin and accumulate memory after disconnect. This causal chain remains a hypothesis pending controlled session-level reproduction. Next: reproduce with bounded clock/disconnect instrumentation, then separate any playable clock repair from historical diagnostic behavior and ensure disconnect remains cooperative. No engine repair or original-executable parity claim made.

## 2026-09-08 — repair midnight PAUSE and disconnect allocation spin

Runnable milestone: a playable DOCK spanning UTC midnight returns to command input, or finishes ordinary scoring/ship release when disconnected during the wait. No existing hosts restarted.

Bounded reproduction connects raw PAUSE to GameSession: the pre-repair driver exhausts 128 retries before a scheduled cancellation can execute, while the wait fixture retains each HIBER operand/event. Both Austin and CompuServe fail this check before the repair (`logs/parity-stall-midnight/before-fix.txt`). This reproduces the mechanism consistent with the original DOCK timestamp, subsequent disconnect timeout and saved heap OOM; there is no original heap snapshot proving every retained allocation in that process.

Source references: Austin WARMAC.MAC:3373-3401 and CompuServe fortran 1978/WARMAC.MAC:4010-4042 compute a raw MSTIME deadline. Neither corrects midnight rollover in PAUSE. New `src/runtime/live-wait.ts`, bound by the production session factory, supplies a monotonic clock for playable PAUSE only, preserving its source cap, retries and variant-specific locking. Historical diagnostic PAUSE and DAYTIM/ETIM retain UTC time-of-day bindings. Live HIBER no longer appends to fixture event/operand arrays. `src/runtime/session.ts` allows disconnect to wake the current wait but schedules subsequent delays, including those with queued input, and yields for closed input waits. Forced shutdown remains able to cancel the job. No larger heap limit or original source edits.

Validation commands and artifacts:
- `node --test test/midnight-session.test.ts` before repair: 2 expected failures, retained in before-fix.txt.
- `node --test test/midnight-session.test.ts test/session.test.ts test/wait-runtime.test.ts`: 71 passed, focused-tests.txt.
- `node --test test/playable-game.test.ts`: 9 passed, playable-tests.txt; two new production DOCK tests simulate wall-clock rollover and verify prompt/STATUS continuation or normal disconnect cleanup.
- `npm test`: 4574 passed, full-tests.txt, including real loopback Telnet regressions.
- Final closed/queued-input refinement and two additional cases: `node --test test/midnight-session.test.ts test/session.test.ts test/playable-game.test.ts test/wait-runtime.test.ts`: 82 passed, final-focused-tests.txt.
- `npm run typecheck`: passed, typecheck-final.txt. Initial test generator type error retained in typecheck.txt and corrected with an explicit generator signature.
- `npm run audit:check`: passed, audit.txt; source/generated archives unchanged.

Current status, compatibility table, playable decision and parity guide updated. Tests establish TypeScript behavior, not native-executable midnight parity or an exhaustive heap audit. Local commit only; earlier publication approval remains unresolved. Other specification work stays unstaged. Next useful check is a longer isolated bot run on the repaired build; no need to wait for real midnight to exercise the boundary.

## 2026-09-08 — broader post-midnight validation in progress

Active user goal: keep testing until reasonable coverage. Isolated ten-ship Austin playable run (seed 1729, balanced versus objective, torpedo corridor, 180 seconds) completed with exit 0, 628 decisions, 487 moves, 40 phaser shot attempts, 3 torpedo attempts, 10 docks, 4 confirmed captures, 12 confirmed builds, no deaths, stalls or reconnect attempts. Evidence: logs/post-midnight-validation/fleet-10/ (launcher, health, transcripts and summary). No existing game host restarted. 99 automated-player/player-library/parity tests passed in experimental-tests-final.txt; initial sandbox EPERM failures retained in experimental-tests.txt.

Memory follow-up: identified isolated host PID 61767 and sampled RSS from about 399 to 820 MiB (memory.jsonl). RSS is not live heap. New isolated diagnostic experimental/parity/memory-host.ts measures post-GC heap and retained-array lengths without changing game services; heap-records.jsonl showed four active captains growing to 50.4 MiB and falling to 17.4 MiB after all sessions ended. Largest record array was a duplicate character transcript (runtime.f.emitted, 71674 entries for one captain). Live OUTCHR previously delegated to a fixture whose only operation was appending that copy (test/fixtures/output-runtime.ts:45). Removed that duplicate capture in the live factory; real terminal output and per-character cooperative yields remain. 39 production-session tests pass (output-retention-tests.txt), type checking passes (typecheck.txt). Matched-duration after-change diagnostic is still in progress. Other fixture event arrays remain; do not claim an exhaustive retention fix.

Extended Austin midnight testing: initial attempt to reuse the CompuServe helper failed because Austin startup/input waiting differs; those edits were removed precisely after inspecting the diff. Whole-file restoration was rejected by automatic review as potentially destructive; no user work was overwritten. Existing Austin Telnet helper now exercises successful midnight DOCK and disconnect cleanup. Its broader combined test then times out at a subsequent Captain0 login, AFTER both affected sessions report completed and release their ships; investigation is ongoing, not a confirmed runtime regression. Diagnostic transcripts are retained in austin-telnet-midnight*.txt. Current test changes and memory tool are not committed yet.

## 2026-09-08 — post-midnight validation checkpoint complete

Completed the bounded coverage review in docs/testing-checkpoint.md. Final `npm test`: 4576/4576 passed (logs/post-midnight-validation/full-tests-pass.txt). Player/library/parity suite: 99/99 passed (experimental-tests-final.txt). Typecheck and audit passed (typecheck-final.txt, audit-final.txt). Actual transport-byte comparison now captures expected bytes locally at SessionTerminal.write; it no longer depends on permanent production fixture output retention. The first full run's resulting assertion failure is retained in full-tests-final.txt; after correcting the test boundary, 17 transport/playable tests and the entire suite pass.

Austin extended Telnet verification is green: midnight DOCK returns to STATUS, disconnect during DOCK releases the ship, then a fresh galaxy supports 18 captains and cleanup. The diagnostic timeout occurred at Captain0's next login: terminal explicitly displayed Regular or Tournament, while the old zero-action fixture supplied an existing-galaxy response. Test now supplies fresh-game setup for that first captain. No additional timing/runtime defect was found in this path; all temporary diagnostic wrappers were removed from the test.

The 60-second, four-captain after-change run completed cleanly. Sampled maximum post-GC heap was 36.7 MiB versus 50.4 MiB before; workloads completed 120 versus 128 decisions, so this is not an exact percentage benchmark. Duplicate emitted-character array entries went from a sampled 71674 for one captain to zero. Other diagnostic event arrays still grow during active sessions; both runs reclaim session storage after quit. Summary: logs/post-midnight-validation/heap-comparison.json, with raw before/after samples and fleet transcripts retained. The final terminal-record removal changes no game text, scheduling yields or original archive bytes.

Three identified temporary host PIDs (61767, 63170, 63462) are absent; ps output is saved in temporary-server-check.txt (empty output/status 1 means no matching live processes). Existing user/reference hosts were not restarted. A whole-file restoration was previously blocked by automatic review; only inspected, assistant-authored test hunks were subsequently removed with apply_patch. No unrelated specification edits were touched or staged.

Reasonable coverage for this testing request is now evidenced across raw clock boundaries, both live variants, Telnet modes/interrupts, 18-captain concurrency, ten autonomous opposing ships, score/ownership/supply effects, cleanup, source audit and memory measurement. This does not certify all native parity or overnight stability. Next maintenance priority is making remaining optional diagnostic event retention explicit/bounded for long-lived sessions; then a longer unattended soak. Publication remains pending the earlier destination approval; commit is local only.

## 2026-09-08 — optional bounded diagnostic histories in progress

User authorized removing remaining diagnostic-record growth. Reviewed fixture append sites and found history-only events, calls, hits, random draw observations, copy/write observations and interrupt observations. Working queues, input/refill buffers, game memory, disk contents and scripted fixture inputs are excluded. Explicit `diagnosticRecords()` allocations replace only reviewed history declarations; creation-time policy is scoped through AsyncLocalStorage, not a process-wide mutable flag. Live createGameSession defaults to limit 0; connected test wrapper explicitly selects 100000; standalone unit fixtures retain ordinary full arrays. Optional host flag --diagnostic-records selects 0..100000 per history. Limits are independent of playable versus historical diagnostic mode.

The first bounded-array implementation shifted the oldest entry and caused the full HELP regression to exceed its timeout after filling a large trace. Retained evidence: logs/diagnostic-retention/full-tests.txt. Replaced shifts with constant-time ring replacement and array-compatible ordered reads. Focused tests cover zero retention, multiple wraps, batched appends, nested/session isolation, invalid limits, ordinary array iteration/JSON/inspection and explicit clearing. The HELP integration assertion formerly sliced an ever-growing trace; now observes its two file-open calls locally, preserving the assertion despite intentional history truncation. Failed intermediate ring assertion retained in ring-tests.txt. Eleven diagnostic-record tests pass in record-tests.txt; final full suite is running.

Measured isolated servers: disabled-memory.jsonl (PID 67347, ten bots/120 seconds), ring-bounded-memory.jsonl (PID 67460, four bots/60 seconds). So far all 95 exposed histories per active session are zero with recording off, and no history exceeds eight with recording enabled. Earlier unused bounded host PID 67368 was stopped before replacement. No existing host restarted. Logs and converted-file inventory are under logs/diagnostic-retention/. Changes not committed yet.

## 2026-09-08 — diagnostic history retention verified

Runnable milestone achieved: live sessions default to zero diagnostic history retention; optional --diagnostic-records / createGameSession diagnosticLimit retains up to the newest N entries separately per history, with 0..100000 accepted. Histories are created explicitly through src/runtime/diagnostic-records.ts and retain a per-array policy selected during composition. Capped histories use constant-time ring replacement with source-ordered array inspection. Standalone fixture histories stay ordinary full arrays; connected test fixtures explicitly select bounded capture. Source action queues, pending messages/hits, input/refill buffers, source stacks, memory/file contents and scripted numeric inputs were not converted. This is host/test instrumentation, not a game-rule or historical-source repair.

The reviewed allocation inventory is in logs/diagnostic-retention/converted-files.json. The live memory diagnostic observes 95 tagged histories per captain, including nested calls/hits/random observations and interrupts. Production default-off run: 10 bots, 120 seconds, 422 decisions, no stalls/errors, all history lengths remain 0, all 10 sessions complete. Cap-eight run: 4 bots, 60 seconds, 121 decisions, no stalls/errors, no history exceeds 8, all 4 sessions complete. Post-GC sample peaks approximately 35.1 and 25.6 MiB; not a performance percentage comparison or proof of every possible allocation. Raw samples, per-bot transcripts and summary.json are retained under logs/diagnostic-retention/. All temporary hosts (67347, 67368, 67460) exited; empty server-exit-check.txt/status 1 is the ps no-match result. Existing hosts were left alone.

Final validation:
- npm test: 4589 passed, full-tests-final.txt.
- node --test experimental/automated-player/test/*.test.ts experimental/player-library/test/*.test.ts experimental/parity/test/*.test.ts: 99 passed, experimental-tests.txt.
- node --test test/diagnostic-records.test.ts: 11 passed, record-tests.txt (wraps, ordered inspection, batched pushes, policy isolation, off/invalid/clear behavior).
- npm run typecheck: passed, typecheck-final.txt.
- npm run audit:check: passed, audit.txt (135 preserved hashes and both variant evidence checks).
- CLI help and out-of-range rejection recorded in cli-help.txt and cli-invalid.txt; invalid limit exits 2 before starting a server.

Initial O(N) shifting caused a HELP timeout; ring replacement resolved it. The subsequent HELP assertion assumed unlimited history; it now captures only its two file-open calls locally, retaining the same ordered-open assertion without relying on historical entries surviving truncation. Both failed runs remain as evidence. Runtime docs now explain diagnostic-only scope, default off, capped opt-in, per-history entry rather than byte limits, and separation from historical diagnostic mode and file logging. Prior checkpoint's growth warning links to this resolution. Detailed logs are optional; no game data or source behavior was removed to reduce memory.

Local commit only, preserving unrelated specification edits and the earlier unresolved publication approval. Existing servers require restart to load the new default; no running game was interrupted.

## 2026-09-08 — restart updated player server on 2423

User requested restart. Process/listener inspection found no TypeScript DECWAR server running; existing PDP-10 reference PID 37470 on 2030 was left untouched. Started Austin playable build a5c5ec9 on 127.0.0.1:2423 using data/automated-player-experiment. Initial launch found .host.lock naming PID 40692; process.kill(pid,0) returned ESRCH. Rechecked ownership and preserved the stale lock at logs/server-restart-a5c5ec9/stale-host-lock.json before retrying. No data files removed.

Verified listening event with diagnosticLimit:0 and source variant Austin, received the correct name prompt over TCP, and closed the probe without commissioning a ship; session-end reports completed. Evidence: logs/server-restart-a5c5ec9/server.jsonl, restarted-console.txt, banner-check.txt; initial failure retained in console.txt. Live exec session 66076 remains running. Next recommended validation is a longer isolated ten-ship soak with memory, combat/death/reentry and final cleanup review, followed by remaining ship-assistance parity cases.

## 2026-09-08 — longer endgame evaluation started

User approved the longer isolated run and asked whether bots can take a game to completion. Started ten ships, five per faction, 1800 seconds, tournament seed 1729, objective strategy on both sides and torpedo corridor enabled. Wrapper exec session 55355; isolated endpoint 127.0.0.1:51677; evidence logs/endgame-evaluation-a5c5ec9/run/. User's Austin server on 2423 remains separate and running.

Bots can fight, repair/resupply, capture/develop planets, attack installations and reenter after ship loss. They lack a demonstrated coordinated endgame and a dedicated war-completion outcome in player.ts (it currently distinguishes decisions/life limits, blocked and interrupted outcomes). Austin DECWAR.FOR:970-971 requires no remaining planets and at least one side with no bases before ENDGAM proceeds. Killing ships or leading fixed-time points is insufficient. Source MSG.MAC:54 emits THE WAR IS OVER!!, with following victory/defeat text. Review actual terminal evidence for this distinction; a timer expiry must not be reported as a completed war. This run is an evaluation of current strategy, not a promise that it will finish a war.

## 2026-09-08 — ENERGY and TRACTOR native comparison complete

User questioned waiting for the 30-minute battle before assistance testing and authorized proceeding. These checks are independent; launched isolated TypeScript host 51791 and native SIMH clone 2031 while the ten-ship battle continued on 51677. User server 2423 and original reference 2030 were preserved. Both assistance clients completed normal quit/logout; test host PID 68607 received SIGTERM after ownership verification and clone exec session 3085 stopped at SIMH prompt and quit. Native console preserved in logs/parity-assistance/native-console.log.

Added experimental/parity/capture-assistance.ts and review-assistance.ts. Two friendly public Telnet clients run sixteen recorded steps with STATUS before/after; no game memory edits. Generalized the existing scan-based safe setup route to either faction, retaining the Empire default. Route safety tests now exercise both factions. Source scope: ENERGY.FOR:29–105 and TRACTR.FOR:27–132, existing implementations unchanged.

logs/parity-assistance/report.json reports matched-observed-effects: all 16 action responses match after command-echo removal, and all measured energy changes, shield modes, before/after positions and hull changes match. Each of two transfers consumes 100 displayed units and gives 90. Both shield restrictions, activation, towing (12 energy, receiver moves into donor's old sector), shield-triggered break, reactivation, OFF and inactive OFF match. Distinct receiver spawn/routes left unequal starting supplies; report preserves them rather than comparing absolute energy. The initial step label energy-capacity is only a repeated transfer here: neither receiver reached capacity, so no clamp is claimed. Recipient notification byte comparison, distant/enemy/inactive targets and other break causes remain open. Native ran SIMH, not Docker. No cooperative bot ENERGY/TRACTOR policy added.

Validation: 19 parity tests pass (parity-tests.txt); both-faction route tests pass (route-tests.txt); production typecheck and scoped experimental typecheck pass (typecheck.txt, experimental-typecheck-retry.txt); source audit passes (audit.txt). Initial scoped compiler command required TypeScript 7 --ignoreConfig; retained invocation error in experimental-typecheck.txt. Raw captures typescript.jsonl and pdp10.jsonl include successful cleanup; report-progress.json also completed successfully. All paths under logs/parity-assistance/. Current status and parity guide updated. Unrelated specification edits untouched.

Battle snapshot at 426 seconds: 1,577 decisions, six captures, 24 build confirmations, no deaths/stalls/base shots/planet shots. This is progress, not a completed war. Existing heartbeat remains responsible for final battle review; isolated battle and user server continue running. Next strategy question is whether ships progress from capturing/building and skirmishing to removing enemy installations and satisfying the actual endgame conditions. No push attempted.

## 2026-09-08 — baseline endgame strategy review

User authorized tracing installation target selection, retreat/resupply loops and war-result detection while leaving the current run unchanged. Review saved at logs/endgame-strategy-review/review.md, reproducible snapshot script analyze.py, structured snapshot.json and analysis.txt. At health timestamp 02:02:20.574Z (711 seconds), ten ships had 2627 decisions, 18 base-shot attempts, zero planet shots, two deaths, nine captures, 28 build confirmations, zero new bases and zero transport stalls. This updates the earlier no-base-attacks observation; no war completion observed.

Found ship combat/shared sightings precede siege; objective captains prioritize capture/development, while full friendly base capacity prevents fifth BUILD. There is no fleet-level endgame plan for the remaining four-build planets. Repeated docking often repairs real damage; Wraith also replenished under incoming fire. Immediate adjacent DOCK bypasses alternative-refuge selection. Talon spent 83 STATUS decisions/92 seconds with unchanged position, energy 103.5, damage and stardate, awaiting assistance that no bot currently provides. Connection health does not detect that strategic stall.

Reviewed actual ENDGAM source legacy/utexas/DECWAR.FOR:970–1004 and MSG.MAC:54–65: terminal messages, points/free/exit, not a required pregame prompt. client/player/supervisor lack explicit war-result handling; do not equate generic complete, death or disconnect with victory. Recommended implementation order: terminal victory/loss recognition with no reconnect; persistent siege objectives and team assignment; strategic-progress/rescue and damaged-refuge decisions. Preserve raw observations, distinguish shot attempts from destruction, and test changes in an isolated subsequent run. This round is read-only runtime analysis plus evidence/log writing, so no gameplay regression suite was rerun and no running process was modified. Existing assistance edits and unrelated specification work remain untouched.

## 2026-09-08 — terminal war results and persistent siege policy

User authorized implementing the endgame review findings. Runnable milestone: player/client/supervisor/fleet can finish on an observed war result, and the next isolated match can select a persistent siege policy. Current 30-minute baseline on 51677 and user server on 2423 remain unchanged; no game rules or archived bytes modified.

Added war-result.ts using Austin DECWAR.FOR:970–1004 and MSG.MAC:54–65. Require banner plus outcome, distinguish Federation/Empire/mutual loss, preserve terminal text and final points, and give play() outcome war-over plus winner. Fleet records warResult, stops commissioning captains and supervisor does not reconnect terminally finished players. Ordinary death reentry remains separate. Truncated banner/outcome is an investigation error, not a retryable disconnect. Drain final text through EOF/native monitor return, with a bounded deadline fallback for a peer that never closes; a fallback may retain only the final output received by that deadline.

Actual production ENDGAM fixture initially failed because the live high-segment notice precedes the banner on the same line: [DECWAR high segment removed from swapper]THE WAR IS OVER!!. Parser now accepts that exact known prefix, not arbitrary message prefixes. Failure and source transcript retained: logs/endgame-policy/production-war-tests.txt and logs/automated-player-scenario-1788919803783-ea0ec5f65199a8.jsonl. The corrected actual Austin Telnet fixture preserves Federation winner and Total points. This is staged test-only game state, not an autonomous victory or original-executable parity. Initial strip-only Node rejection of a TypeScript parameter property is also retained in war-tests.txt; replaced with ordinary fields.

Opt-in mode siege retains a known enemy base/developed planet for up to five minutes across resupply; current contrary scan/ownership evidence invalidates it. Fresh SCAN authorizes firing; LIST/remembered positions only authorize navigation. Uses existing source-informed five-sector base/three-sector planet standoff and strength-180 phasers. Ships within four sectors take defensive priority; more distant skirmishes do not displace a safe siege action. Fleet strategy siege assigns one objective captain per faction and siege mode to the rest. Defaults unchanged. No team-wide assignment or full-capacity planet endgame solver is claimed.

Added strategic-state observation separately from transport health: 30 seconds with unchanged position, energy, shields, damage, torpedoes and stardate produces one strategic-stalled event; state changes emit strategic-resumed. Join/reentry resets the watch. Health exposes counts/current flag. This reveals Talon's responsive low-energy waiting pattern, but does not yet provide ENERGY/TRACTOR rescue or alter docking/retreat decisions. Useful repeated repairs are not treated as stalls.

Validation under logs/endgame-policy/: broad player/library/parity suite 111 passed (experimental-tests.txt); subsequent fleet/war/progress integration 12 passed (final-focused-tests.txt), including four-ship siege roster and terminal fleet stop; final client/war/progress/captain regressions 43 passed (final-regressions.txt); actual runtime/split-message war tests 8 passed (production-war-tests-retry.txt); scoped strict erasable TypeScript check passes (typecheck-final.txt); source audit passes (audit.txt). Tests cover target persistence through resupply, removal, fresh firing evidence, nearby ship defense, ordinary death, EOF/monitor/chunked final output and no reconnect. No production game-code change, so the entire game suite was not rerun.

Updated player guide, quickstart and status with delivered behavior and remaining limitations. Existing heartbeat review-decwar-one-hour-fleet-run updated through the app: after the baseline actually finishes, review it then launch exactly one isolated 600-second ten-ship siege-versus-siege run, seed 1729, torpedo corridor, logs/endgame-siege-evaluation/run. Check manifest/process to prevent duplicates. Then review raw installation progress and terminal result and pause heartbeat. Preserve user2423/reference2030. Same seed does not align different subsequent random draws. No push attempted; unrelated specification work remains untouched.

## 2026-09-08 — baseline finished; siege follow-up launched

Status check at 02:23 UTC found the baseline completed at 02:20:31 UTC with fleetExitCode 0 and durationReached true. Final baseline: 7249 decisions, three deaths, 18 base-shot attempts, zero planet-shot attempts, 14 confirmed captures, 50 build increments, zero new bases and zero transport stalls. Raw captain transcript search found no THE WAR IS OVER!! banner. Full base capacity constrains interpretation of zero new bases; this is not evidence of a build failure. Old baseline did not contain the newly added strategic-stall instrumentation.

No running fresh-fleet/fleet process and no siege output directory existed at the duplicate check. Launched the already-authorized ten-ship, 600-second siege-versus-siege follow-up using seed 1729 and torpedo corridor. Exec session 62098; evidence logs/endgame-siege-evaluation/console.txt and run/. This starts a separate temporary galaxy; user2423 and reference2030 preserved. Existing heartbeat is configured to recognize this manifest and monitor rather than duplicate it, review installation progress/results, then pause after completion.

## 2026-09-08 — siege midpoint confirms installation destruction

Heartbeat review around 02:29:30 UTC, six minutes into the authorized siege run: all ten captains playing, 1458 decisions, 107 base-shot attempts, zero planet shots, five captures, 16 build confirmations, four new bases, no deaths/transport stalls/strategic stalls. Raw action-result transcripts confirm eight base destructions by about 02:29:33: four Federation and four Empire bases, distinct coordinates. Scout converted two planets and Raven converted two, freeing the corresponding planets from the remaining-planet count. Both sides' latest friendly base lists showed eight bases, consistent with ten initial minus four destroyed plus two constructed. Evidence preserved at logs/endgame-siege-evaluation/six-minute-review.json with raw destruction responses and conversion events; live logs continue growing.

This is meaningful endgame progress, unlike merely increased shooting: destroying bases opens friendly replacement capacity for the other side, allowing objective captains to convert planets. No war banner/result yet; no victory claim. Baseline had 18 base-shot attempts over 30 minutes versus 107 in the first six minutes here, but trajectories/random draws and exposure differ, so this is an observational comparison rather than controlled causal parity. Keep current siege run unchanged until its ten-minute limit, then review final results and pause heartbeat as configured. No new match launched by this heartbeat.

## 2026-09-08 — siege evaluation final review

While running independently authorized ENERGY edge parity, found the siege follow-up completed at 02:33:32 UTC with launcher fleetExitCode 0. Final ten-minute siege: 2325 decisions, 156 base-shot attempts, zero planet-shot attempts, 11 captures, 36 build increments, eight planet-to-base conversions, no deaths and no transport or strategic stalls. Raw action-result messages confirm eleven base destructions. Last friendly base counts are Federation eight/Empire nine, consistent with 20 initial minus eleven destroyed plus eight constructed. No raw war banner and no terminal warResult; the duration ended, not the war.

Compared with the unchanged thirty-minute baseline: 7249 decisions, 18 base-shot attempts, zero destruction messages and zero conversions; all friendly base lists remained at ten. Siege base-shot rate was 15.6/minute versus baseline 0.6/minute, descriptive only because duration, trajectories and later random draws differ. Eight planets removed through conversion is actual end-condition progress. Neither run attempted planet phasers; full-capacity handling and finishing the remaining planets still need evaluation. Preserved reports, raw destruction responses, conversion events and launcher outcomes in logs/endgame-siege-evaluation/final-comparison.json. Paused existing heartbeat through the app after this authorized final review; no additional match launched. Independent ENERGY edge captures continue on separate test instances. User2423 and reference2030 preserved.

## 2026-09-08 — ENERGY capacity and recipient parity verified

User authorized further parity work independently of the siege evaluation. Extended capture-assistance.ts with opt-in energy-edges scenario. Three ordinary public Telnet clients, no hidden-state deposits: YORKTOWN donor, VULCAN recipient, then live enemy WOLF. Before approach test distant and inactive recipients. Route recipient using fresh public scans to 7-3, restore at adjacent base, make two one-sector shielded moves to create exactly sixteen units of capacity, then test oversized transfer, full recipient, negative amount and enemy recipient. Source: ENERGY.FOR:29–105 (scaled capacity and integer division) and existing movement/docking behavior. Standard sixteen-step assistance scenario remains supported.

Both live captures completed cleanly; logs/parity-energy-edges/report.json reports matched-observed-effects for all six action responses (after command echo removal) and explicit contracts. Clamp: receiver 4984 to 5000; donor 5000 to 4982.3, a 17.7-unit debit. Full-recipient request reports transfer but changes neither supply. Distant/inactive/negative/enemy requests reject with unchanged supplies/positions. Distant setup positions differ but independently satisfy nonadjacency. No game code changed and no general numeric/compiler or random-stream parity claim. Native reference is SIMH, not Docker.

Reviewer now requires correct edge commands, complete three-client cleanup, actual capacity preconditions and source-derived changes; matching wrong arithmetic cannot pass. Twenty-one parity tests pass (parity-tests.txt), strict scoped typecheck passes (typecheck-final.txt), source audit passes (audit.txt). Reanalysis of the previous sixteen-step capture still matches (previous-capture-review.json). Raw wire/action/status captures typescript.jsonl and pdp10.jsonl, host memory/lifecycle output and native-console.log retained in logs/parity-energy-edges/.

Isolated TypeScript host PID72181 on54406 stopped after verified ownership and client cleanup. Native clone2031 stopped at SIMH prompt then quit; original2030 and user2423 preserved. Siege final review completed and its heartbeat paused separately above. Current guide/status updated. Remaining useful parity work includes additional TRACTOR break causes and recipient notification bytes; cooperative rescue remains strategy work rather than missing command support.

## 2026-09-08 — extended tractor parity verified

User requested the next checks. Added tractor-edges mode to the public assistance capture plus review-tractor-edges.ts. Corrected initial expectation from source before testing: moving either attached ship tows its counterpart rather than necessarily breaking the beam (MOVE energy/reciprocal-position path). Source references: TRACTR.FOR:27–132, Austin DECWAR.FOR FREE:1099 and SHIELD:3793; existing game code unchanged.
